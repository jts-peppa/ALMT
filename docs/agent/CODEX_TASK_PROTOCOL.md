...
## R2 Formal Baseline Boundary

R2 is a separate owner-approved task for MOSI aligned, Seed 1111. It preserves the upstream fixed 200-epoch schedule rather than introducing early stopping, selects the checkpoint with best Valid MAE, and evaluates Test exactly once only after training completes. The workflow supports epoch-boundary resume from a matching local checkpoint, rejects source/config/seed drift, and commits only `EXP-002`, the experiment index, and project status. A single-seed result remains `PRELIMINARY`; further seeds require new approved tasks.

## Reviewed PR Merge Boundary

Automatic merging requires a separate human-applied `merge-approved` label. The label must be applied last by `jts-peppa` to an open, non-draft PR targeting `master`. Eligible authors are `jts-peppa`, or `github-actions[bot]` only when the PR comes from the same repository and a branch explicitly produced by a guarded experiment workflow (`codex/almt-gpu-smoke-*` or the fixed R2 report branch). The current PR head must have exactly one owner-authored comment containing `<!-- ALMT-RESEARCH-REVIEW -->`, the matching `Source PR` and full `Source Commit`, and the independent field `PR Disposition: MERGE`; these fields may use the fixed plain-text or Work Markdown-list representation. Research decisions such as `CONFIRMED`, `PRELIMINARY`, `FAILED`, or `INCONCLUSIVE` do not authorize a merge. Forks, other bot branches, a changed head, ambiguous review, requested changes, or an unmergeable PR stop the workflow.
...
An owner-created `ALMT-TASK-*` Issue may trigger the restricted documentation executor only after the owner applies `approved`. The task must identify an open same-repository Source PR and exact Source Commit and list one to ten explicit Markdown files under `docs/agent/`; list entries may be plain paths or Markdown-code paths. `NEXT_TASK.md`, `PLAN.md`, and `DECISIONS.md` remain protected. Codex runs with workspace-write access but is instructed not to execute training, inference, dataset, network, Git, or GitHub commands; the trusted wrapper performs Git operations and rejects every changed path outside the exact allowlist. Successful work updates the existing PR, performs Draft-to-Ready, comments on and closes the task Issue, and relies on Work to re-review the new commit.
